...
namespace Assigment
{
    public partial class Form1 : Form
    {
        ...
        private void SendMail(string email)
        {
            ...
            {
                content = "You lated " + delayFormatted + "Môn Prn211 slot 2, Lớp của thầy Hoàng ";
            }
            ...
            try
            {
                ...
                foreach (var Column in values)
                {
...

                    {
                        ...
                        var attendanceTime = new DateTime(date.Year, date.Month, date.Day, 6, 45, 0); // Thời gian điểm danh là 6 giờ 45 sáng
                        var endTime = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, 11, 45, 0); // Thời gian kết thúc là 11 giờ 45 sáng
                        ...
                        if (date >= attendanceTime && date <= endTime)
                        {
                            ...
                            if (delay.TotalMinutes <= 15)
                                ...
                                status = "Late"; // Đến trễ
                            var rowNumber = i;
                            var range = $"{"Object"}!F{rowNumber}:G{rowNumber}";
                            var ValueRange = new ValueRange();
                            ValueRange.Values = new List<IList<object>> { new List<object> { date.ToString("HH:mm:ss"), status } };
                            var updatePlank = service.Spreadsheets.Values.Update(ValueRange, "1Wcg80UudlfC-m_HnTKlfVA7InbNofRCUP9G0BBaVx0M", range);
                            ...
                            SpreadsheetsResource.ValuesResource.GetRequest getRequest = service.Spreadsheets.Values.Get("1Wcg80UudlfC-m_HnTKlfVA7InbNofRCUP9G0BBaVx0M", "Object!A:G");
                        ...
                        else if (date > endTime)
                        {
                            ...
                            ValueRange getRespone = getRequest.Execute();
                            ShowForm show = new ShowForm(studentCode);
                            MessageBox.Show($"Đã kết thúc lớp học, Hãy gửi email cho thầy Hoàng để được điểm danh ");
        ...
        private void button1_CheckedChanged(object sender, EventArgs e)
        {
            ...
            mjp.ParseStream(new Uri("http://172.20.10.8:4747/video?640x480"));
